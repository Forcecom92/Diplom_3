from locators import reset_password_locators as rpl
...
class ResetPasswordPage(BasePage):
    ...
    def click_button_enter_account(self):
        self.click(rpl.BUTTON_ENTER_ACCOUNT)

    @allure.step('клик по кнопке "Восстановить пароль')
    def click_reset_password(self):
        self.click(rpl.BUTTON_RESET_PASSWORD)
    ...
    def set_email_input(self, email):
        email_input = self.wait_and_find_element(rpl.EMAIL_FIELD)
        email_input.send_keys(email)

    @allure.step('Клик по кнопке "Восстановить"')
    def click_button_restore_password(self):
        self.click(rpl.RESTORE_BUTTON)
